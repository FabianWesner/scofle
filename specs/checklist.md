...
- [x] Multi-image upload creates one queued Conversion per image and redirects to the first created Conversion.
...
- [x] Inline PDF preview renders for ready Attempts with `pdf_bytes` present when PDF rendering is enabled.
...
- [x] Uploading another image or batch creates separate Conversions in the same session.
...
- [x] Delete-all removes DB rows and files for every Conversion in the current session without touching other sessions.
...
- [x] A direct multi-file request consumes one nonce for the whole batch; client-side sequential batch uploads rotate the nonce after each accepted image.
...
- [x] `php artisan conversions:reap` is scheduled every 10 minutes with overlap protection.
...
- [x] Reaper deletes orphaned temporary conversion directories that no longer have database rows.
...
- [x] LibreOffice PDF rendering is opt-in via `CONVERSION_RENDER_PDF=true`; disabled preview leaves the Attempt ready with `.pptx` downloadable, and PDF failure leaves the Attempt ready with `failure_code='pdf_render'`.
...
- [x] Session-wide delete requires current-session ownership and deletes only that session's Conversions.
...
- [x] Inline PDF responses use `Content-Disposition: inline` and `Content-Security-Policy: sandbox` when PDF rendering is enabled.
...
- [x] Queue processing allows multiple pending Attempts but converts only one Attempt at a time in FIFO order.
...
- [x] Upload, pending/running, ready, partial, failed, rate-limited, offline/slow-network, delete-confirm, and delete-all states are implemented.
- [x] Recent Conversions show status indicators for waiting, processing, done/partial, and failed states.
...
- [x] Feature tests cover happy upload, batch upload queueing, FIFO processing, downloads, regenerate, second image upload as separate Conversion, session-only access, delete, validation failures, lifecycle, rate limits, signed downloads, and failure-code mapping.
...
- [x] Playwright Chrome QA covers upload, multi-image batch queueing, polling, regenerate, second image upload, attempt switching, refresh/back/double-submit behavior, delete confirmation, delete-all, PDF-preview unavailable state, focus rings, console-error checks, and session-only URL access.
...
- [x] Upload both example images in one batch and verify PPTX artifacts are produced. PDF artifacts are verified only when `CONVERSION_RENDER_PDF=true`.
...
- [x] Delete all session Conversions; sidebar becomes empty and other sessions remain untouched.
